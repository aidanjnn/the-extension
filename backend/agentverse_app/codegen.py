...
from agentverse_app.deterministic_codegen import (
    build_deterministic_files,
    build_hardcoded_demo_files,
)
...
async def run_codegen(request: CodegenRequest) -> CodegenResult:
    ...
    hardcoded = build_hardcoded_demo_files(
        query=spec.behavior,
    ...
    if hardcoded:
        ...
        files, use_case_id = hardcoded
        hardcoded_issues = _quality_issues(
            files,
            spec.target_urls,
            spec.behavior,
        ...
        if hardcoded_issues:
            logger.warning(
                "Hardcoded demo %s produced quality warnings: %s",
                use_case_id,
                hardcoded_issues,
            )
        source = f"hardcoded:{use_case_id}"
    ...
    if files is None and deterministic:
    ...
    if files is None and not attempted_llm:
        files = await _generate_checked(
            query=spec.behavior,
            target_urls=spec.target_urls,
            extension_name=spec.name,
